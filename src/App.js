...
export default function App() {
  ...
  // Use the latest entered shift month as the active monthly tracker month
  // so totals automatically roll over when a new month entry is added.
  const latestEntryDate = projectEntries.length
    ? new Date(
        Math.max(
          ...projectEntries.map(([key]) => {
  ...
  const currentMonthKey = getMonthKey(latestEntryDate);

  const monthlyMins = projectEntries.reduce((sum, [key, entry]) => {
    const datePart = key.split("__")[1];
    const entryDate = new Date(datePart);
    return getMonthKey(entryDate) === currentMonthKey
