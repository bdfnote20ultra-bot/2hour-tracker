...
const THEMES = {
  ...
  classic: {
    ...
    appBg: "#F7F5F2", headerBg: "#1C1C1E", statsBg: "#2C2C2E", cardBg: "#FFFFFF", modalBg: "#F7F5F2", text: "#1C1C1E", headerText: "#F7F5F2", muted: "#888888", line: "rgba(0,0,0,0.10)", buttonBg: "#1C1C1E", buttonText: "#F7F5F2", shadow: "0 1px 4px rgba(0,0,0,0.06)", font: "Georgia, serif"
  },
  oled: {
...
export default function App() {
  ...
  }, [musicSettings.volume, musicData]);
  ...
  const toggleMusic = async () => {
    ...
    try {
      ...
      setIsMusicPlaying(false);
      alert("Your browser blocked autoplay. Tap Play again after choosing a song.");
    }
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: pageBackground, backgroundSize: "cover", backgroundPosition: "center", backgroundAttachment: "fixed", fontFamily: theme.font, maxWidth: 480, margin: "0 auto", color: appTextColor, transition: "background 0.25s ease, color 0.25s ease" }}>
      <audio ref={musicRef} src={musicData || undefined} loop onEnded={() => setIsMusicPlaying(false)} />
      {/* Header */}
      <div style={{ background: theme.headerBg, color: theme.headerText, padding: "52px 24px 16px", position: "sticky", top: 0, zIndex: 10, ...glassStyle }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          ...
          <button
            ...
            style={{ background: "none", border: "none", color: vibrantLabelColor, cursor: "pointer", fontSize: 22, fontWeight: 900, textShadow: readableTextShadow }}
          >
            {view === "settings" ? "✕" : "⚙"}
          </button>
        </div>
        {view === "week" && (
          <>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
              <button onClick={() => setWeekOffset(w => w - 1)} style={{ background: "none", border: "none", color: vibrantLabelColor, cursor: "pointer", fontSize: 22, fontWeight: 900, padding: "4px 8px" }}>‹</button>
              <div style={{ textAlign: "center" }}>
                <div style={{ fontSize: 13, color: vibrantLabelColor, fontFamily: "system-ui", fontWeight: 900, textShadow: readableTextShadow }}>
                  {weekOffset === 0 ? "This Week" : weekOffset === -1 ? "Last Week" : `${Math.abs(weekOffset)}w ${weekOffset < 0 ? "ago" : "ahead"}`}
                </div>
                <div style={{ fontSize: 12, color: vibrantMutedColor, fontFamily: "system-ui", fontWeight: 800, textShadow: readableTextShadow }}>
            ...
            {musicData && (
              <>
                <div style={{ fontSize: 13, color: appTextColor, fontFamily: "system-ui", fontWeight: 800, marginBottom: 10, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                  Current: {musicSettings.fileName || "Selected song"}
                </div>
            ...
            return (
              ...
              >
                <div style={{ width: 52, flexShrink: 0 }}>
                  ...
                  <div style={{ fontSize: 11, color: today_ ? vibrantMutedColor : vibrantMutedColor, fontFamily: "system-ui", fontWeight: 800, textShadow: readableTextShadow }}>{formatDate(date)}</div>
                </div>
